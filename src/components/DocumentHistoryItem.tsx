...
const DocumentHistoryItem: React.FC<DocumentHistoryItemProps> = ({
  ...
  const handleDownload = async () => {
    try {
      // Check if the activity has a file path associated with it
      if (activity.filePath) {
        toast.info(`Preparing download: ${title}`);
        const downloadUrl = await getFileURL(activity.filePath);

        // Create a link and trigger download
        const a = document.createElement('a');
        a.href = downloadUrl;
        a.download = activity.filePath.split('/').pop() || 'document';
