...
import { ParsedCV, exportCV } from "@/services/documentParser";
...
const CVExporter: React.FC<CVExporterProps> = ({ cvData, onExport }) => {
  ...
  const handleExport = async () => {
    ...
    try {
      setIsExporting(true);
      await exportCV(cvData, format, template);
        ...
        {format === 'pdf' && (
          <div className="space-y-4">
            <div className="text-center py-4 border-b">
              <h1 className="text-2xl font-bold">{cvData?.personalInfo?.name || 'Full Name'}</h1>
              <p className="text-gray-600">{cvData?.personalInfo?.title || 'Position Title'}</p>
            ...
            {cvData?.publications && cvData.publications.length > 0 && (
              <div>
                <h2 className="text-lg font-bold border-b mb-2">Publications</h2>
...
\\end{document}`}</pre>
  ...
  return (
    <Card>
      ...
      <CardContent>
        <Tabs defaultValue="pdf" value={format} onValueChange={(value) => setFormat(value as 'pdf' | 'word' | 'latex')}>
          ...
          <div className="mb-4">
            <label className="text-sm text-gray-500 mb-2 block">CV Template</label>
            ...
            <Dialog open={showPreview} onOpenChange={setShowPreview}>
              ...
              <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
                <h2 className="text-xl font-bold mb-4">
                  {format.toUpperCase()} Preview - {template.charAt(0).toUpperCase() + template.slice(1)} Template
                </h2>
                {renderPreview()}
              </DialogContent>
            </Dialog>
